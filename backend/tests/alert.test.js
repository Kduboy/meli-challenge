...
describe('GET alarms with query params', () => {
    ...
    test('Should get alarms like description', async () => {
        const response = await request(app)
            .get('/alerts?description=do')
            .send()
            .expect(200);

        const alerts = response.body.rows.filter(
            (alert) => !alert.description.includes('do')
        );
    ...
    test('Should get alarms by server name and like description', async () => {
        const response = await request(app)
            .get('/alerts?server=Server2&description=do')
            .send()
            .expect(200);
